<template>
  <div class="app-container">
    ...
    <!-- 使用模板对话框 -->
    <el-dialog v-model="useTemplateDialogVisible" title="使用模板" width="70%">
      <div>
        <p>
          您正在使用模板 <strong>{{ selectedTemplate?.name }}</strong>
        </p>
        ...
        <el-form :model="useTemplateForm" label-position="top">
          ...
          <template v-if="useTemplateForm.action === 'create'">
            ...
            <el-form-item label="Git 仓库">
              <el-input
                v-model="useTemplateForm.git_repo"
            ...
            <el-form-item label="Git 分支">
              <el-input
                v-model="useTemplateForm.git_branch"
                placeholder="请输入Git分支，默认为main"
              />
            </el-form-item>
          </template>
          ...
          <template v-else>
            <el-form-item label="选择流水线">
              <el-select
                v-model="useTemplateForm.pipeline_id"
                placeholder="请选择流水线"
              >
                <el-option
...
const fetchTemplates = async () => {
  ...
  try {
    ...
    const response = await templateApi.getBuildTemplates(params);
...
const viewTemplate = async (id) => {
  ...
  try {
    const response = await templateApi.getBuildTemplateById(id);
...
const saveTemplate = async () => {
  ...
  try {
    if (templateForm.id) {
      ...
      // 创建模板
      await templateApi.createBuildTemplate(templateForm);
